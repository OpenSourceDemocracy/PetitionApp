...
export class OrbitManager {
  static async createOrbit(directory='./orbitdb', options?: any){
    let ipfs: IPFS;
    if(options && options.temp){
      ipfs = await DefaultIpfs.createTemp(options)
    }else{
      ipfs = await DefaultIpfs.create(options);
    }
    return new Orbit_db(ipfs, directory, options);
  }
}
